...
const Layout = () => {
    ...
    const { authLogoutUrl } = getEnv();
    const handleLogout = () => {
        if (authLogoutUrl) {
            openExternal(authLogoutUrl);
        } else {
            navigate('/');
        }
    };
                ...
                <div className='backdrop-blur-md bg-white/40 border border-white/30 rounded-2xl p-3 shadow-[0_0px_10px_0px_rgba(0,0,0,0.05)]'>
                    <div className='flex items-center justify-between overflow-x-auto scrollbar-hide'>
                        ...
                        <div className='flex gap-1 ml-2 flex-shrink-0'>
                            <div onClick={() => alert('Settings clicked')} className='p-2.5 cursor-pointer flex items-center justify-center rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-200 transition duration-300'>                        
                                <Settings className='w-5 h-5'/>
                            </div>
                            <div onClick={handleLogout} className='p-2.5 cursor-pointer flex items-center justify-center rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-200 transition duration-300'>                        
            ...
            <div className='flex flex-col sm:flex-row flex-1 overflow-hidden'>
                ...
                <div className='hidden sm:flex flex-col w-20 md:w-50 h-full bg-[#f5f5f5] m-4 rounded-[30px] p-1 justify-start items-center shadow-[0_0px_10px_0px_rgba(0,0,0,0.05)]'>
                    ...
                    <div className='mt-auto mb-4 flex flex-col gap-2'>
                        <div onClick={() => alert('Settings clicked')} className='px-4.5 py-2.5 cursor-pointer flex items-center gap-4 rounded-2xl text-sm font-medium text-gray-600 hover:bg-gray-200 transition duration-300 hover:scale-105'>                        
                            <Settings className='w-5 h-5'/>
                            <div className="hidden md:inline">Setting</div>
                        </div>
                        <div onClick={handleLogout} className='px-4.5 py-2.5 cursor-pointer flex items-center gap-4 rounded-2xl text-sm font-medium text-gray-600 hover:bg-gray-200 transition duration-300 hover:scale-105'>                        
                            <LogOut className='w-5 h-5'/>
