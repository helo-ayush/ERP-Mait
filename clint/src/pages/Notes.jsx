...
import { getEnv, apiFetch } from '../utils/env';

const Notes = () => {
  const [activeNoteType, setActiveNoteType] = useState('text');
  const { apiBaseUrl, filesBaseUrl } = getEnv();
  const handleAddNote = async () => {
    try {
      await apiFetch('/notes', { method: 'POST', body: JSON.stringify({ type: activeNoteType, title: 'Untitled' }) });
      alert('Note action sent (mock). Implement backend to persist.');
    } catch (e) {
    ...
    <div className="flex-1 p-4 md:p-6 overflow-y-auto scrollbar-hide">
      ...
      <div className="mb-6 backdrop-blur-md bg-white/70 rounded-2xl border border-white/20 shadow-xl p-4 md:p-6">
        ...
        <div className="flex items-center justify-end mt-4">
            <button onClick={handleAddNote} className="px-5 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors font-semibold flex items-center gap-2">
