...
const AdminDashboard = () => {
  ...
  const [pendingTeachers, setPendingTeachers] = useState(() => read('teachersPending', []));
  const [approvedTeachers, setApprovedTeachers] = useState(() => read('teachersApproved', []));
  ...
  useEffect(() => {
    setPendingTeachers(read('teachersPending', []));
    setApprovedTeachers(read('teachersApproved', []));
  ...
  const handleApproveTeacher = (teacherId) => {
    ...
    const newApproved = {
      ...
      classes: classes.filter(c => c.teacherEmail === t.email).length,
      students: classes.filter(c => c.teacherEmail === t.email).reduce((a, c) => a + (c.students?.length || 0), 0),
      avatarInitial: (t.name || 'T').charAt(0).toUpperCase(),
    };
    const updatedApproved = [...approvedTeachers, newApproved];
    setPendingTeachers(updatedPending);
    setApprovedTeachers(updatedApproved);
    write('teachersPending', updatedPending);
    write('teachersApproved', updatedApproved);
  };
