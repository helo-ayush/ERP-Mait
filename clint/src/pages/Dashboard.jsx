            ...
            <div className="flex-1 min-h-0">
              <div className="h-full overflow-y-auto scrollbar-hide">
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  {[
                    { label: 'Request Certificate', icon: ExternalLink, color: 'blue', onClick: () => openExternal(`${apiBaseUrl}/certificates/request`) },
                    { label: 'Apply for Leave', icon: Calendar, color: 'green', onClick: () => openExternal(`${apiBaseUrl}/leave/apply`) },
                    { label: 'Raise Ticket', icon: AlertCircle, color: 'orange', onClick: () => openExternal(`${apiBaseUrl}/support/ticket/new`) },
                    { label: 'View Hostel Info', icon: Home, color: 'purple', onClick: () => navigate('/students/hostel') }
                  ].map((action, index) => (
                    <button
                      key={index}
