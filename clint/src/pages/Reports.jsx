  ...
  const ReportCard = ({ report }) => (
    <div className="backdrop-blur-md bg-white/70 rounded-2xl border border-white/20 shadow-xl p-6 hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] flex flex-col justify-between">
      ...
      <div className="flex items-center gap-2 mt-6">
        <button className="flex-1 px-4 py-2 bg-white/60 hover:bg-white/90 text-sm font-semibold text-gray-800 rounded-lg transition-colors flex items-center justify-center gap-1">
          <Download className="w-4 h-4" />
          Export
        </button>
        <button className={`flex-1 px-4 py-2 bg-${report.color}-500 hover:bg-${report.color}-600 text-sm font-semibold text-white rounded-lg transition-colors flex items-center justify-center gap-1`}>
          View Details
